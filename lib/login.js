...
const defaultOptions = {
  ...
  fail: noop,
  loginUrl: null,
  wxwork: wx.getSystemInfoSync().environment === 'wxwork'
}

const loginFn = function () {
  return defaultOptions.wxwork ? wx.qy.login : wx.login
}
...
function login (opts) {
  ...
  getWxLoginResult((err, loginResult) => {
    ...
    const header = {
      ...
      [constants.WX_HEADER_QY]: opts.wxwork ? 'true' : 'false'
    }
...
function initLogin (opts) {
  defaultOptions.loginUrl = opts.loginUrl

  if (opts.wxwork !== undefined) defaultOptions.wxwork = opts.wxwork
}

module.exports = { login, setLoginUrl, loginWithCode, initLogin }
